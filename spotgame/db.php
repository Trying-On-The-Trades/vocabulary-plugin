...
function get_all_spotgame_words($db, $deck_id, $right_word_id){
    ...
    $rows = $db->query(
        "SELECT wdic.word, wdic.description, wdic.points, wdic.audio, wdic.image
              ...
              WHERE wdec.id = {$deck_id} AND
                    wdic.id <> {$right_word_id}");
...
function get_word($db, $deck_id, $right_word_id){
